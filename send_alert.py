...
def send_alert(service, object, subject, content, gmsgto='', gmailto='', auto_merge="1"):
    """service 产品线下的服务,object 服务下的某个部分,subject 报警的标题，短信的内容 """
    if not service or not object or not subject or not content:
        ...
        response = client.post('/v1/alert/send', data)
...
def send_warning(service, group, msg='alert test'):
    object_ = msg
    datetime = time.strftime("%Y-%m-%d %X", time.localtime())
    subject = datetime
    content = '%s:%s  %s' % (service, msg, datetime)
    send_alert(service, object_, subject, content, group, group, '1')


def usage(return_code=2):
    help = """
Usage %s --sv "DIP" --service "DIP-Service" --object "object" --subject "subject" --gmsgto "somegroup" [options]
      %s --service "DIP_KAFKA" --subject "Critical. Kafka Service 10.13.xx.xx is failed. "  --content "alert msg content." --gmsgto "DIP_TEST"
...
if __name__ == "__main__":
    ...
    for field in ['sv', 'service', 'subject']:
        if not data[field]:
            print '%s could not be null!!!' % field
            usage()

    if data['mailto'] and data['msgto'] and data['ivrto'] and data['weiboto'] and\
       data['gmailto'] and data['gmsgto'] and data['givrto'] and data['gweiboto']:
        print 'Give one contacts atleast!!!'
        usage()

    m = re.match(
        'https?:\/\/(?P<hostname>[^\/:?]+)(?::(?P<port>\d+))?(?P<path>[^?]*)(?:\?(?P<query>\S+))?', data.pop('url'))
